...
<body>
  ...
  <script>
    ...
    function getComponentSource(component) {
      ...
      const meaning = morpheme?.source_meaning ?? parsedOrigin.meaning ?? morpheme?.core_meaning ?? component.meaning_in_word;
    ...
    function morphemeList(ids) {
      const unique = [...new Set((ids ?? []).filter(Boolean))];
      if (!unique.length) return '<p class="note">No entries yet.</p>';
      return '<ul class="link-list">' + unique.map((id) => {
        const morpheme = morphemeById.get(id);
        return '<li>' + (morpheme ? routeButton('morpheme', id, morpheme.display) : escapeHtml(id)) + '</li>';
      }).join('') + '</ul>';
    }
    ...
    function renderMorpheme(morpheme) {
      const meanings = morpheme.senses.map((sense, index) =>
        '<li class="sub-panel" style="margin-bottom:12px"><strong>' + (index + 1) + '. ' + escapeHtml(sense.meaning) + '</strong><span class="note"> ' + escapeHtml(sense.zh) + '</span><div style="margin-top:8px">' + wordList(sense.examples) + '</div></li>'
      ).join('');
      const connectedWords = words.filter((word) => word.components.some((component) => component.morpheme === morpheme.id)).map((word) => word.word);

      document.getElementById('detail').innerHTML =
        '<header><h1>' + escapeHtml(morpheme.display) + '<span class="meta">' + escapeHtml(morpheme.type) + '</span></h1><p class="definition">' + escapeHtml(morpheme.core_meaning) + '</p><p class="zh">' + escapeHtml(morpheme.zh) + '</p></header>' +
        detailSection('Origin', '<p class="note">' + escapeHtml(morpheme.origin) + '</p>') +
        detailSection('Top Meanings', '<ol style="margin:0;padding-left:22px">' + meanings + '</ol>') +
        detailSection('Common Patterns', '<ul class="dense-list">' + (morpheme.patterns ?? []).map((item) => '<li>' + escapeHtml(item) + '</li>').join('') + '</ul>') +
        detailSection('High-Value Words', wordList(morpheme.high_value_words)) +
        '<div class="grid-2">' +
          detailSection('Similar-Looking Roots', morphemeList(morpheme.similar_form)) +
          detailSection('Meaning-Neighbor Roots', morphemeList(morpheme.similar_meaning)) +
        '</div>' +
        detailSection('Confusable Words', '<ul class="dense-list">' + (morpheme.confusable_words ?? []).map((item) => '<li>' + escapeHtml(item) + '</li>').join('') + '</ul>') +
        detailSection('Connected Words', wordList(connectedWords));
    }
